...
def nondim_cr3bp(t, Y, pi_2, t0s, t0j, rs, rj, pi_s, pi_j): ## Every input is a non dimensional entity
    ...
    Ydot[3] = 2 * ydot + x - (1 - pi_2) * (x + pi_2) / sigma**3 - pi_2 * (x - 1 + pi_2) / psi**3 + pi_j*X_j/rj**3 #+ pi_s*X_s/rs**3 
    Ydot[4] = -2 * xdot + y - (1 - pi_2) * y / sigma**3 - pi_2 * y / psi**3 + pi_j*Y_j/rj**3 #+ pi_s*Y_s/rs**3 
    Ydot[5] = -(1 - pi_2)/sigma**3 * z - pi_2/psi**3 * z ## Assuming only planar motion, no Perturbation here
    #print((1 - pi_2) * (x + pi_2) / sigma**3, pi_s*X_s/rs**3,pi_j*X_j/rj**3,pi_s*Y_s/rs**3, pi_j*Y_j/rj**3)
    return Ydot

def nondim_cr3bp_without_pert(t, Y, pi_2, t0s, t0j, rs, rj, pi_s, pi_j): ## Every input is a non dimensional entity
    #print(rs,rj,pi_s,pi_j)
    ang_pos_s = t+t0s
    ang_pos_j = t+t0j
    X_s = rs*m.cos(ang_pos_s)
